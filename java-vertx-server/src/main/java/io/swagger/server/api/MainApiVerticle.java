...
public class MainApiVerticle extends AbstractVerticle {
    ...
    public void deployVerticles(Future<Void> startFuture) {
        ...
        vertx.deployVerticle("io.swagger.server.api.verticle.SumApiVerticle", res -> {
            if (res.succeeded()) {
                LOGGER.info("SumApiVerticle : Deployed");
            } else {
                startFuture.fail(res.cause());
                LOGGER.error("SumApiVerticle : Deployment failed");
            }
        });
        
        vertx.deployVerticle("io.swagger.server.api.verticle.UpperApiVerticle", res -> {
            if (res.succeeded()) {
                LOGGER.info("UpperApiVerticle : Deployed");
            } else {
                startFuture.fail(res.cause());
                LOGGER.error("UpperApiVerticle : Deployment failed");
            }
        });
